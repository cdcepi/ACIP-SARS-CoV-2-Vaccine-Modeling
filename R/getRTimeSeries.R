...
getRTimeSeries <- function(model, actual = FALSE) {
  if (actual) { #Calculate from derivatives
    ...
    if(class(Model)=="SEAIRTVModel")
    {
      startDays=model$parameters$ContactMatrixStart+1
      for(i in 1:length(startDays))
        {
        contactMatrixArray[, , startDays[i]:min(startDays[i+1],nrow(susceptibilityVectorTimeSeries),na.rm=T)] = model$parameters$ContactMatrices[[i]]
        }
    ...
    effectiveRTimeSeries <-  unlist(lapply(1:nrow(susceptibilityVectorTimeSeries), function(x) {
      ...
      })) * model$parameters$beta / ifelse(class(Model)=="SEAIRTVModel",1/((1/model$parameters$lambda2)+(1/model$parameters$gamma)),model$parameters$gamma)
    
    if(class(Model)=="SEAIRTVModel")
    {
      if(model$parameters$Seasonality)
      {
        beta_amp = model$parameters$R0_amp / 
          max(Mod(eigen(
            ((1/model$parameters$gamma) + 1 / model$parameters$lambda2) * model$parameters$contactMatrix,
            symmetric = FALSE, only.values = TRUE
          )$values))
        
        beta_Max =  model$parameters$beta + beta_amp*0.5 - 0.5*beta_amp*cos((2*pi/(52*7))*(model$parameters$dayOfR0-(model$parameters$offset)))
        if(length(beta_amp)!=1 ) {stop(paste("length(beta_amp!=1, length"))}
        if(length(beta_Max)!=1)  {stop("length(beta_max!=1")}
        if(length(model$parameters$offset)!=1 ) {stop("length(model$parameters$offset!=1")}
            
         beta_time = 0.5*beta_amp*cos((2*pi/(52*7))*((1:length(effectiveRTimeSeries))-(model$parameters$offset))) + (beta_amp/2)+beta_Max-beta_amp
       # if(length(beta_time)!=1){stop(paste0("length beta_time != 1, length beta_time = ",length(beta_time)))}
         beta_time
        
        effectiveRTimeSeries = effectiveRTimeSeries * beta_time / model$parameters$beta
      }
    }
...
getInfectiousnessVectorTimeSeries.SEAIRTVModel <- function(model) {
  ATimeSeries = getCombinedTimeSeries(model, "A")
  AvTimeSeries = getCombinedTimeSeries(model,"Av")
  AvbTimeSeries = getCombinedTimeSeries(model,"Avb")
  ITimeSeries <- getCombinedTimeSeries(model, "I")
  IvTimeSeries <- getCombinedTimeSeries(model, "Iv")
  IvbTimeSeries <- getCombinedTimeSeries(model, "Ivb")
  IAVax1TimeSeries = IvTimeSeries + AvTimeSeries
  IAVax2TimeSeries = IvbTimeSeries + AvbTimeSeries
  IAVaxAnyTimeSeries = IAVax1TimeSeries + IAVax2TimeSeries
  IATotalTimeSeries = ITimeSeries + IvTimeSeries + IvbTimeSeries + ATimeSeries + AvTimeSeries + AvbTimeSeries
  return(sweep(ifelse(IAVaxAnyTimeSeries > 0,
                      1 - (model$parameters$VEi1 * IAVax1TimeSeries / IATotalTimeSeries) - (model$parameters$VEi2 * IAVax2TimeSeries / IATotalTimeSeries),
                      1),
               2, 1 - model$parameters$AVEi.eff, "*"))
