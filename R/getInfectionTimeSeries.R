...
getInfectionTimeSeries.SEIRV2DoseModel <- function(model, byGroup = TRUE, asRate = FALSE, incidence = FALSE,
  ...
  if (incidence) { #Incidence
    ...
    if (symptomatic) {
      ...
      dailyInfectionsByGroup <- getCombinedTimeSeries(model, c(getCompartments(model, type = "E"),
                                                                    getCompartments(model, type = "A"),
                                                                    getCompartments(model, type = "I")))
    }
    colnames(dailyInfectionsByGroup) <- getLabels("prevalence", compartmentLength)
  }
...
#' @export
#' 
getInfectionTimeSeries.SEAIR_NPI_Model<- function(model, byGroup = TRUE, asRate = FALSE, incidence = FALSE,
                                             symptomatic = FALSE, fractionSymptomatic, byWeek = FALSE) {
  compartmentLength = length(model$parameters$populationFractions)
  if (incidence) { #Incidence
    if (symptomatic) {
      ...
      cumulativeIncidenceByGroup <- getCombinedTimeSeries(model, append(getCompartments.SEAIR_NPI_Model(model, type = "I"),
                                                                        getCompartments.SEAIR_NPI_Model(model, type = "R")))
      dailyInfectionsByGroup <- t(t(rbind(rep(0, compartmentLength),
                                          cumulativeIncidenceByGroup[2:nrow(cumulativeIncidenceByGroup), , drop = FALSE] -
                                            cumulativeIncidenceByGroup[1:(nrow(cumulativeIncidenceByGroup) - 1), , drop = FALSE])) *
                                    fractionSymptomatic)
    } else {
      dailySusceptiblesByGroup <- getCombinedTimeSeries(model, getCompartments.SEAIR_NPI_Model(model, type = "S"))
      dailyInfectionsByGroup <- rbind(rep(0, compartmentLength),
                                      dailySusceptiblesByGroup[1:(nrow(dailySusceptiblesByGroup) - 1), , drop = FALSE] -
                                        dailySusceptiblesByGroup[2:nrow(dailySusceptiblesByGroup), , drop = FALSE])
    }
    ...
    if (symptomatic) {
      ...
      dailyInfectionsByGroup <- t(t(getCombinedTimeSeries(model, getCompartments.SEAIR_NPI_Model(model, type = "I"))) * fractionSymptomatic)
    } else {
      dailyInfectionsByGroup <- getCombinedTimeSeries(model, append(getCompartments.SEAIR_NPI_Model(model, type = "E"),
                                                                    getCompartments.SEAIR_NPI_Model(model, type = "I")))
    }
    colnames(dailyInfectionsByGroup) <- getLabels("prevalence", compartmentLength)
  }
  makeOutputFromDailyInfectionsByGroup(model, dailyInfectionsByGroup, byGroup, asRate, byWeek)
